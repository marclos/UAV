...
You can now access your Pi via a monitor; however, if you would access your pi via a computer, follow the guide for "Option 3: Remote Access on a Configured Pi". 

\end{enumerate}

\subsection{Option 2: Remote ssh Access}
...
\item Ensure you have the pre-configured boot SD card installed in the Pi.

\item Connect your Pi to power.

\item Open up your computer's shell (on Windows, this can be accessed under apps by typing ``Windows Powershell'' or ``Command Prompt'').

\item To determine if the Pi is communicating with your network, start the command prompt and type:
...
\item Now we need to determine the Pi IP address. For Windows use a program called Nmap; for Macs ...?? 

\item Use Nmap to scan your network by entering the IP address from your computer...

\item The results are not easy to sort through... but 

\begin{figure}
\includegraphics[width=\textwidth]{images/2-2_Nmap_results}
\caption{Getting the Pi IP address with Nmap. This screenshot is from the Nmap program -- Notice the circles that identify the Rasbian OS, the Raspberry Pi, and linux opperating system. Here the IP address is 192.168.0.3}
\label{fig:2Nmap}
...
\item Unplug the Pi and remove the SD card then insert the Pi's SD card into your computer and access it through your computer's file navigator.  

It should be accessible under ``BOOT'' and ``RECOVERY''.

\item If the RECOVERY file directory exists, open the ``OS'' folder.  Within OS, there may be multiple system options in different folders; delete all of them except for ``Raspian''. This configures your operating system to Raspian.

\item Go back to the root (or the main level) of the BOOT drive. This is where we will create the files that will allow you to operate via SSH and connect to wifi.

\item Create or open a document named ``wpa\_supplicant.conf'' as described below. 

\begin{description}
  \item[Mac instructions (wifi settings)] Create a new empty file that will hold network info:

\begin{lstlisting}
touch /Volumes/boot/wpa_supplicant.conf
\end{lstlisting}

\item[Windows instructions (wifi settings)] Run Notepad. Paste in the contents below (adjusting for the name of your country code, network name and network password). Click File / Save As. Be sure to set Save as type to All Files (so the file is NOT saved with a .txt extension). Call the file wpa\_supplicant.conf and save it. Close the file.
\end{description}
...
\item Next, if there is no file called, create a file named ``ssh'' using one of the following methods:
